...
```{r , message=FALSE, warning = FALSE, cache=TRUE, echo=FA:SE}
...
```{r , echo=TRUE, createDirs, warning=TRUE}
...
**Note: eval = F**
```{r subsetFxn, eval = TRUE, echo=TRUE}
...
```{r defineParams, eval = T, warning=T, message=F}
...
```{r calcMetrics, eval=TRUE, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r importCalcMetrics, message=F, warning=F, echo=F}
#  Import EWS results
results_ews <-
...
Plot the distance travelled, *s* for transect # 13:
```{r }
...
p.s <- sort.year.line(plotResults, metric.ind, year.ind, dirID.ind, dirInd,  scale = T, center = T)
    # +
    # # place a v-line at the location of Fort Riley Military base
    #  geom_vline(aes(xintercept=-96.8), color = "grey", linetype = 2)
p.s
```


Plot the velocity of the distance travelled, *dsdt* for transect # 13:
```{r }
